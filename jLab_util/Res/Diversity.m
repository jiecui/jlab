...
%           
%
%     Usage: [divers,evenness,S,dCI,eCI,dSE,eSE] = ...
...
%                       evenness indices.
%         dSE =       [1 x C] vector of estimated standard errors for diversity indices.
%         eSE =       [1 x C] vector of estimated standard errors for evenness indices.
%
...
function [divers,evenness,S,dCI,eCI,dSE,eSE] = diversity(freqs,kind,N,iter,CI_level)
  ...
  if (isvect(freqs))                      % Convert vector of freqs to col vector
    freqs = freqs(:);
  end;
