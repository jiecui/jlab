...
%
%     Usage: [loadings,percvar,scores,fscores,CI_loadings,CI_percvar,evects]
...
%         scores =      [n x ndf] matrix of discriminant scores (columns).
...
%         evects =      [p x p] matrix of eigenvectors.
%
...
%   1/4/04 -   return eigenvectors.

function [loadings,percvar,scores,fscores,CI_loadings,CI_percvar,evects] ...
            = discrim(X,grps,ndf,loadtype,Xf,norescale,iter,CI_level)
...
%   ndf = min([ndf,ngrps-1,nvars]);         % Set number of functions returned
  ...
  [loadings,percvar,scores,fscores,B,W,evects] = discrimf(X,grps,Xf,ndf,loadtype,norescale);
