...
%           of N into k group-sample sizes, omitting permutations.  Does not permit
%           zero sample sizes.
%
...
% RE Strauss, 8/29/99
%   6/19/04 - added error message for k>N.

function sizes = allsizes(N,k)
  if (k>N)
    error('  Allsizes: number of groups must be <= total sample size.');
  end;
