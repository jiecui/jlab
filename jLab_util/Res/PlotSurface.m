...
function [X,Y,Z] = plotsurface(x,y,z,plottype,ngrid,ptsymbol,extrapts,...
  ...
  plotpoints = 0;
  if (~isempty(ptsymbol))   plotpoints = 1; end;
  if (isempty(ngrid))       ngrid = [40,40]; end;
  if (length(ngrid)==1)     ngrid = [ngrid,ngrid]; end;
  if (isempty(plottype))    plottype = 1; end;
  if (isempty(notsmoothed)) smoothed = 0; end;
  if (isempty(contours))    contours = 0; end;
  if (isempty(color))       color = 0; end;
