...
%   12/10/04 - allow 'nvect' to be optional.

function [loadings,scores,meanscr,wvar] = loadscrs(X,evects,nvect,loadtype,zs)
  if (nargin < 1) help loadscrs; return; end;
  
  if (nargin < 3) nvect = []; end;
  if (nargin < 4) loadtype = []; end;
  if (nargin < 5) zs = []; end;

  [n,p] = size(X);

  if (isempty(nvect))    nvect = p; end;
  if (isempty(loadtype)) loadtype = 0; end;
  if (isempty(zs))       zs = 0; end;
  ...
  scores = score(X,evects,nvect);           % Scores for subset of eigenvectors
