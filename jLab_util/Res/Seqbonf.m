...
%    5/24/05 - changed names of boolean flags
...
function [signif,critlim] = seqbonf(pvalues,alpha,independent)
  ...
  if (isempty(independent)) independent = 0; end;
  ...
  if (is_row_vector)                     % Transpose for row vector
    signif = signif';
  end;

  if (is_sqsymmet)                       % Reconstruct square symmetric matrix
