...
%           values =    optional [m x 1] vector of numeric values (e.g., allometric
%                         coefficients) for each distance measure,
%                         or [n x c] character-string matrix of labels.
...
%   12/10/03 -  corrected problem in default 'individ' vector.
...
function [xv,yv,v] = plotdist(crds,dists,individ,regpts,title,values,digits,fontsize)
  ...
  if (isempty(digits))   digits = 3; end;
