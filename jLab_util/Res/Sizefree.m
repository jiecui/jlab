...
%  1/2/04 -   set lower limit of 1 for 'ndf'.
...
function [loadings,percvar,scores,fscores,D2,R,wload,wperc,wsize,S] ...
  ...
  if (ndf < 1)                          % Set number of discriminant functions
    ndf = 1;                            %   within bounds
  elseif (ndf > max_ndf)
    ndf = max_ndf;
  end;
  ...
  if (~ischar(kindsize))
    error('  Sizefree: invalid kind of size vector.');
  end;
...
%                        10) S:        among-group size scores      [n x 1]
  ...
  outsize = [nvars*ndf, ndf, nobs*ndf, nfobs*ndf, ngrps*ngrps, nobs*nvars, nvars, 1, nobs, nobs];
  i = find(~outmat);
  if (~isempty(i))
