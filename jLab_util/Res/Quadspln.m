...
%
%     Usage: [scrds,indx] = quadspln(crds,{start},{doplot},{nsp})
%
...
%           indx =    [n x 1] vector of indices of 'scrds' closest to each point in
%                       'crds'.
%
...
%   2/4/05 -  output indices of 'scrds' corresponding to original 'crds'.

% Bowyer, A & J Woodwark. 1983. A programmer's geometry.  Butterworths, London.

function [scrds,indx] = quadspln(crds,start,doplot,nsp)
  if (nargin < 1) help quadspln; return; end;
  ...
  if (get_indx)                               % Find boundary pts closest to original pts
    n = size(save_crds,1);
    indx = zeros(n,1);
    for ip = 1:n
      d = eucl(save_crds(ip,:),scrds);
      [dmin,indx(ip)] = min(d);
    end;
  end;
