% BootModelParams: Tests for statistical differences in model coefficients among groups, 
%         based on parameter estimates from a model fitted independently to each group.
...
%                        BootModelParams(params,stderrs,corrs,N,{iter},{noplot},{alpha})
%
...
%         ---------------------------------------------------------------------------------
%         F_tot =       F-statistic value for pooled manova.
...
function [F_tot,pr_tot,F_pair,pr_pair,signif_pair] = ...
                               BootModelParams(params,stderrs,corrs,N,iter,noplot,alpha)
  if (nargin < 1) help BootModelParams; return; end;
  if (nargin < 4) error('  BootModelParams: too few input parameters.'); end;
  ...
  if (~isequal(size(params),size(stderrs)))
    error('  BootModelParams: incompatible parameter and standard-error matrices.');
  end;
  ...
  if (length(N)==1)
    N = N*ones(k,1);
  elseif (length(N)~=k)
    error('  BootModelParams: invalid sample-size vector.');
  end;
  
  if (ndims(corrs)==2)                 % Convert 2D corrs matrix to 3D
    corrs = corrs(:);
    if (length(corrs)~=k)
      error('  BootModelParams: invalid parameter-correlation matrix.');
    end;
    c = ones(p,p,k);
