...
function handles = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend,ax)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	handles.bars = bar(ax,barvalues, width); hold(ax)
	if length(bw_colormap)
		colormap(ax,bw_colormap);
	else
		colormap(ax,jet);
	end
	...
	for i = 1:numbars
		...
		handles.errors(i) = errorbar(ax,x, barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none');
	end
	set(gcf, 'color', 'white')

	if ~isempty(bw_title)
		handles.title = title(ax,bw_title, 'fontsize',12);
	else
		handles.title = [];
	end
	if ~isempty(bw_xlabel)
		handles.xlabel =xlabel(ax,bw_xlabel, 'fontsize',12);
	else
		handles.xlabel = [];
	end
	if ~isempty(bw_ylabel)
		handles.ylabel =ylabel(ax,bw_ylabel, 'fontsize',12);
	else
		handles.ylabel = [];
	end

	set(ax, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], 'fontsize', 12, 'xtick',1:numgroups);
	...
	if ~isempty(bw_legend)
		handles.legend = legend(ax,bw_legend, 'location', 'best', 'fontsize',12,'box','off');
% 		legend boxoff;
	else
		handles.legend = [];
	end
