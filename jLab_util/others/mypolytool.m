function [h,betaci] = mypolytool(x,y,n,alpha,xname,yname)
...
%   non-simultaneous 95% confidence interval for a new observation of Y
...
% $Revision: 2.28 $  $Date: 2002/01/17 21:31:43 $
...
if (strcmp(action, 'start'))
   ...
   if (nargin < 2)
      error('POLYTOOL requires at least two arguments.');
   end
   [anybad wasnan y x] = statremovenan(y,x);
   if (anybad==2), error('Lengths of X and Y must match.'); end
   if (any(wasnan(:)))
      warning('Removed (X,Y) pairs where either value was NaN.')
   end
end
...
if ~strcmp(action,'start')
   ...
   deltay = get(y_field(2), 'String'); % will be NaN if string is empty
end
...
stats = dofit(x,y,degree,stats,1);
...
ud.wasnan = wasnan;
ud.texthandle = [];
ud.simflag = 0;        % non-simultaneous confidence intervals
ud.obsflag = 0;        % for new observation, not for mean
ud.confflag = 1;       % show confidence bounds
ud.robflag = 1;        % not a robust fit
...
case 'output',
  ...
  ud.texthandle = uicontrol(dlghandle,'Style','edit','Units','pixels',...
                            'String',entrystr, 'BackgroundColor','w',...
...
case 'toworkspace'
    ...
    switch outval
       ...
       case 7,
	      ...
	      assignin('base',s((cpos(4)+1):end        ),fullresid);
    end
...
if nargout == 2
    ...
    dxtxi = sqrt(sum(Rinv'.*Rinv'));
    dbeta = dxtxi*stats.crit(degree)*rmse;
    betaci=[beta-dbeta; beta+dbeta; beta; dbeta];
end
...
uicontrolHandles.outputpopup=uicontrol('Style','Popup','String',...
        ...
        'CallBack','polytool(''output'')', 'BackgroundColor','w');
