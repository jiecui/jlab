...
%
%   SLICER(IMG, POS, ZOOM, SMOOTH_KERNEL, {SMOOTHED_IMG}): specifies a
%   smoothing kernel and optionally pre-smoothed image
%   
%   requires :
%   readstack
%
...
% Last Modified by GUIDE v2.5 24-Feb-2006 18:17:59
...
handles.grayMap = [0 256];
handles.smoothKernel = gaussfilt(6,2);
handles.smoothToggle = 0;
...
% now the filter kernel
if length(varargin)>3
    handles.smoothKernel = varargin{4};
    handles.smoothCalculated = zeros(dim(3),1);
end

if length(varargin)>4
    handles.smooth_img = varargin{5};
    handles.smoothCalculated = ones(dim(3),1);
    handles.smoothToggle = 1;
else
    handles.smooth_img = zeros(dim,'uint16');
    handles.smoothCalculated = zeros(dim(3),1);
end
...
if handles.color
    ...
    h_img = imshow(handles.img(:,:, zslice),handles.grayMap);
end
...
if handles.color
    ...
    if handles.smoothToggle
        handles = smoothSlice(handles);
        set(handles.h_img, 'CData', handles.smooth_img(:,:,slice));
    else
        set(handles.h_img, 'CData', handles.img(:,:,slice));
    end
end
...
% ------------------------------------------------
function handles = smoothSlice(handles)

% get slice
slice = handles.pos(3);

if ~handles.smoothCalculated(slice)
    handles.smooth_img(:,:,slice) = imfilter(squeeze(handles.img(:,:,slice)),...
        handles.smoothKernel);
    handles.smoothCalculated(slice) = 1;
end
% guidata(handles.mainFrame,handles);
...
%set(handles.imageDisplay, 'xlim', [pos(2)-zoom/2 min(pos(2)-1+view(2)/zoom, dim(2))+zoom/2]);
%set(handles.imageDisplay, 'ylim', [pos(1)-zoom/2 min(pos(1)-1+view(1)/zoom, dim(1))+zoom/2]);
set(handles.imageDisplay, 'xlim', [pos(2)-.5 pos(2)-1+view(2)/zoom+.5]);
set(handles.imageDisplay, 'ylim', [pos(1)-.5 pos(1)-1+view(1)/zoom+.5]);
...
% --- Executes on button press in smooth_toggle.
function smooth_toggle_Callback(hObject, eventdata, handles)
% hObject    handle to smooth_toggle (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of smooth_toggle

if  get(hObject,'Value')
    handles.smoothToggle = 1;
    % check if smooth_data_exists(curframe) is zero in user data
    handles = smoothSlice(handles);
else
    handles.smoothToggle = 0;
    % setslice(curframe) ?
...
% --- Executes on button press in saveslice_button.
function saveslice_button_Callback(hObject, eventdata, handles)
% hObject    handle to saveslice_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

default = [pwd filesep '*.bmp'];
curprefs = getpref('slicer','lastsavepath',default);

% Prompt for saving as image
if curprefs
    [p,n,e,v] = fileparts(curprefs); 
    imgName = [p filesep 'image_' num2str(handles.pos(3)) '.bmp'];
else
    imgName = [pwd filesep 'image_' num2str(handles.pos(3)) '.bmp'];
end
[filename, pathname] = uiputfile(imgName, 'Save Slice as BMP file');
if ~isequal(filename,0) & ~isequal(pathname,0)
    if handles.smoothToggle
        imag = uint8(handles.smooth_img(:,:,handles.pos(3)));
    else
        uint8(handles.img(:,:,handles.pos(3)));
    end
    imwrite(imag,colormap(gray),...
        fullfile(pathname, filename),'bmp');
end
if ~isequal(pathname,0)
    setpref('slicer','lastsavepath',pathname);
end