...
% sessions.
%
% Variable name cannot include '_'.
%
...
% Martinez-Conde at Barrow Neurological Institute in Phoenix, AZ. Each
% variable is stored in a separate file, to make additions and deletions
% very fast. Additionally, there is no penalty for grouping the data by
% variable or by session.

% Last modified by Richard J. Cui on Fri 06/10/2016 11:18:09.972 AM
%
...
% if one wants to access/save data from/to a specific location which may
% not depend on the experiment or subjects or may not belong in the current
% working database then attatch with_location to the command
if length(cmd) > 14
    if strcmp(cmd(end - 12:end),'with_location')
        db_name = varargin{1};
        b = cell(nargin - 2, 1);
        for i = 1:(nargin - 2)  % length(varargin)-1
            b{i} = varargin{i+1};
        end
        varargin = b;
        cmd = cmd(1:end-14);
...
try
    ...
    switch cmd
            ...
            if nargin >= 3
                ...
                if pn
                    ...
                    for i=1:length(varnames)
                        ...
                        dat = sess.(varnames{i});
                        save(fullfile(db_name,fullname), varstr(dat), '-MAT');
                    end
                else
                    return
                end
        ...
        case 'open'
            if nargin > 1
                db_name = varargin{1};
                setpref( 'corrui', 'db_name', db_name );
                % should we check if the file exists here?
            else
        ...
        case 'close'
            clear db_name;
            setpref( 'corrui', 'db_name', '' );
        case 'add'
            % add/update an session, requires nargin >=3
            if nargin >= 3
                sessname = varargin{1};
                sess = varargin{2};
                if ( isempty(sess) )
                ...
                for i=1:length(varnames)
                    ...
                    if ver(1) >= '7'
                        ...
                        save(fullfile(db_name,fullname), varstr(dat), '-v7.3');
                    else
                        save(fullfile(db_name,fullname), varstr(dat));
                    end
                end
            else
        ...
        case 'copysess'
            if nargin >= 3
                ...
                for i=1:length(d)
                    ...
                    varname = oldfilename((strfind( oldfilename, '_' ) + 1):(strfind( oldfilename, '.mat' ) - 1));
                    newfilename = [varargin{2} '_' varname '.mat'];
                    % dos ( ['copy ' db_name '\' filename ' ' db_name '\' fullname]);
                    full_oldfilename = [db_name, filesep, oldfilename];
                    full_newfilename = [db_name, filesep, newfilename];
                    copyfile(full_oldfilename, full_newfilename)
                end
                % toc
            else
                error('ERROR');
        ...
        case 'copysessto'
            if nargin >= 3
                source = [ db_name filesep varargin{1} '_*.mat'];
                dest   = varargin{2};
        ...
        case 'listsess'
            ...
            % d = dir([db_name '\*internalTag.mat']);
            d = dir([db_name filesep '*internalTag.mat']);
            sesss = {};
            j = 1;
            for i=1:length(d)
                sess = d(i).name;
