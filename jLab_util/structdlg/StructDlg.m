...
% Process one Dialog
% ------------------
global rec_level    % recursive level; global var. needs to remember levels
...
elseif ischar(struct_def) % (isstr(struct_def))
...
for i = 1:length(f_units)
    if (ischar(v_units{i}) && ~isempty(v_units{i}))
        ...
        index = find(strcmp(f_units{i},fnames));
        if (~isempty(index))
...
function ud = set_fields_ui(def,h_fig,ud, ~,fnames,ignore_defaults)
%
% vals = def;
...
for i = 1:length(fnames)
    ...
    elseif (iscell(val))
        ...
        elseif length(val) == 2 && ...
               sum(strcmp(val{1}, {'0','{0}'})) > 0 && ...
               sum(strcmp(val{2}, {'1','{1}'})) > 0
    ...
    elseif (isstruct(val))
        if (isfield(ud.units,fnames{i}))
            ...
            sub_units = ud.units.(fnames{i});
        else
            if (isfield(ud.orig_def,fnames{i}))
                % [~, sub_units] = split_def(getfield(ud.orig_def,fnames{i}));
                [~, sub_units] = split_def(ud.orig_def.(fnames{i}));
            else
                sub_units = struct([]);
            end
        end
    ...
    if isempty(h) || ~prod(ishandle(h))  % h did not exist before. Now it is!
...
for i = 1:length(fnames)
    ...
    AU = ud.auto_update.(fnames{i});
    if (~isempty(AU))
        old_vals = ud.vals;
        for j = 1:length(AU)
            ud = set_fields_ui(ud.orig_def,hfig,ud,[],AU(j),ignore_defaults);
        end
...
for i = 1:length(AU_fnames)
    ...
    oldval = old_vals.(AU_fnames{i});
    % val    = getfield(new_vals,AU_fnames{i});
    val    = new_vals.(AU_fnames{i});
...
for i = 1:length(fnames)
    ...
    prev_undo = ud.undo_vals.(fnames{i});
...
function retval = secure_eval(~, str) % retval = secure_eval(this,str)
...
if (~isempty(dflt_val))
    ...
    dflt_selected = find(strcmp(dflt_val,options));
...
if ((~isempty(dflt_val)) && (dflt_val == 0 || dflt_val == 1))
    ...
    selected = find(strcmp({'{0}', '{1}'}, val)) - 1;
    if (isempty(selected))
        selected = 0;
    end
end
...
if (~isempty(dflt_val))
    ...
    dflt_selected = find(strcmp(dflt_val,val));
    if (~isempty(dflt_selected))
        % ud.vals = setfield(ud.vals,f,dflt_val);
        ud.vals.(f) = dflt_val;
        ...
        switch (push_ud.cmd)
            case {'uigetfile','uiputfile','uigetdir'}
                ...
                if (strcmp(push_ud.cmd,'uigetdir'))
                    ...
                    fname = pushudCmd(filter_spec, extra_args);
                    pname = '';
                else
                    % eval(['[fname, pname] = ' push_ud.cmd '(' filter_spec  extra_args ');'])
                    [fname, pname] = pushudCmd(filter_spec, extra_args);
                end
                return_val = '';
                str = '';
            ...
            case 'StructDlg'
                if (isempty(ud.error))
                    ...
                    ret_struct = StructDlg(ud.orig_def.(f),title,dflt_val,rec_pos,'on',v);
