function crunchStruct = collapseSlicesSort(crunchStruct, subject_id, collapsed_info)
...
for i=1:length(groups)
   ...
   if ~isempty(match)
      ...
      insert = collapsed_info{i}{2};
      subj.data.avg = subj.data.avg(:,:,[1:insert  new_avg  insert+1:size(subj.data.avg,3)]);
      subj.data.std= subj.data.std(:,:,[1:insert new_avg insert+1:size(subj.data.std,3)]);
      subj.slice_names = [subj.slice_names{[1:insert]} {' '} subj.slice_names{[size(subj.data.std,3)];
