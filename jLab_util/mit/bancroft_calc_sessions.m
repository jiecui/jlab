function [cond_results, measure_names, condition_names] = bancroft_calc_sessions(trial_data_filename, measure_data_filename, project_files, comments, delay_cells, target_cells)
...
for i=1:length(project_files),
   ...
   if (use_targets & ~isempty(target_cells{i}))
      [tr.results, tr.cond_res, tr.colnames, tr.rownames] = bancroft_proj_calculate(cp,delay_cells{i});
   elseif (use_delays & ~isempty(delay_cells{i}))
         [tr.results, tr.cond_res, tr.colnames, tr.rownames] = bancroft_proj_calculate(cp,delay_cells{i});
	else
      [tr.results, tr.cond_res, tr.colnames, tr.rownames] = bancroft_proj_calculate(cp);
   end
   %check for error
