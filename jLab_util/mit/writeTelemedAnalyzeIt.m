function crunch = writeTelemedAnalyzeIt(crunch, subjnumbers, prestring, laststring, filename)
...
for s=subjs
   ...
   % increasing
   subjrestavg = subj.data.avg(:,:,firstslice:lastslice); 
   subjreststd = subj.data.std(:,:,firstslice:lastslice); 

%    if lastslice-firstslice < 7
%       % then we have to pad restavg with NaN's
%       missingslices = 7-(lastslice-firstslice);
%       pad = ones([size(subjrestavg(:,:,1)) missingslices])*NaN;
...
% % now what about Normals -- hardcode session 22 "all w.o EB"
% norm = crunch.subjects(1).data.avg(:,:,1);
% normdata = [];
...
% 
% % now for std
% norm = crunch.subjects(1).data.std(:,:,1);
% normdata = [];
...
for i=1:length(crunch.measures)
   timelabel = [timelabel {'Pre' 'Last' 'Pre' 'Post15 Rx' 'Post 30 Rx'}];
end
