function[err, idx, errordata] = spatial_error(session, targets, sensor_no)

%     EXAMPLE: err = spatial_error(session, targets, sensor_number)
%
...
err = zeros(length(trial_start),1);
idx = zeros(length(trial_start),1);
for i = 1:length(trial_start)
   ...
   tr = tr( :, 3:5 );
   % switch the trial names to find out if you have a telemed rwt. In that case, project to get a 3:5
   % that are at the end of the mail piece (14 cm)
   %switch session.scene_names{i},
   %case{'RRWTtelemedMAILCENTERdiag.VR','LRWTtelemedMAILCENTERdiag.VR','RRWTtelemedMAILCENTERhoriz.VR','LRWTtelemedMAILCENTERhoriz.VR'}
   ...
   spaterror = sqrt( sum( ( tr - ones( size( tr, 1), 1 ) * targets( i, : ) )'.^2 ) );
   [err(i), idx(i)] = min( sqrt( sum( ( tr - ones( size( tr, 1), 1 ) * targets( i, : ) )'.^2 ) ) );
   errordata{i} = spaterror;
end
