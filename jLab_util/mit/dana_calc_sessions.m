function [cond_results, measure_names, condition_names] = dana_calc_sessions(trial_data_filename, measure_data_filename, project_files, comments, delay_cells, target_cells)
...
for i=1:length(project_files),
    ...
    if (use_targets & ~isempty(target_cells{i}))
        [tr.results, tr.cond_res, tr.colnames, tr.rownames] = dana_proj_calculate(cp,delay_cells{i});
    elseif (use_delays & ~isempty(delay_cells{i}))
        [tr.results, tr.cond_res, tr.colnames, tr.rownames] = dana_proj_calculate(cp,delay_cells{i});
    elseif isfield(cp.session_info, 'delays')
        [tr.results, tr.cond_res, tr.colnames, tr.rownames] = dana_proj_calculate(cp,cp.session_info.delays);
    else
        [tr.results, tr.cond_res, tr.colnames, tr.rownames] = dana_proj_calculate(cp);
    end
    %check for error
