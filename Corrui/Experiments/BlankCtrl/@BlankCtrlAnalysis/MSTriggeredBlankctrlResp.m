function result_dat = MSTriggeredBlankctrlResp(current_tag, sname, S, dat)
% MSTRIGGEREDBLANKCTRLRESP Individual cell MS-triggered response at different luminance level
% 
% Description:
%   This function calculates MS-triggered response of individual cell.  The
%   results are averaged spike rate at each luminance level.  The averaged
%   spike rate can be either absolute spike rates or normalized one,
%   according to the choices. Modified from MSaccContrastAnalysis.MSTriggeredContrastResponse.m
% 
% Syntax:
...
% process
% sort microsaccades into different luminance levels
% -------------------------------------------------
[sorted_usacc, usacc_num] = sortMSContLevel(sname, usacc, paras);      % sort usacc into different contrast level
...
% -----------------------------------------
[fr_base_mean, fr_base_sd] = getBaselinFR(sname, sorted_usacc, timestamps, paras);
...
result_dat.MSTriggeredBlankctrlResp = msresp;
...
function [spk_raster, trl_len] = getBaseSpkRaster(sname, usacc_props, timestamps, paras)
...
function [spk_raster, trl_len, trl_num] = cutBaseSpkRaster(sname, sorted_usacc, timestamps, paras)
...
for k = 1:nlevel
    ms_k = sorted_usacc{k};
    [spk_raster_k, tlen_k] = getBaseSpkRaster(sname, ms_k, timestamps, paras);
...
function [fr_winc, fr_level] = estFRContLevelBootstrp(sname, spk_raster, trl_len, trl_num, paras)

Spike = MSCSpike(sname);    % MSC spike object
...
function [fr_mean, fr_sd, fr_winc, fr_level] = SpkRateBootstrap(sname, spk_raster, trial_length, trial_num, paras)

[fr_winc, fr_level] = estFRContLevelBootstrp(sname, spk_raster, trial_length, trial_num, paras);
...
function [fr_base_mean, fr_base_sd, fr_base_winc, fr_base_fr] = getBaselinFR(sname, sorted_usacc, timestamps, paras)
% baseline fring rate meand and s.d.

% get baseline spike time raster
[spk_raster, trial_length, trial_num] = cutBaseSpkRaster(sname, sorted_usacc, timestamps, paras);
...
switch baselin_method
    ...
    case 'Bootstrap'
        [fr_base_mean, fr_base_sd, fr_base_winc, fr_base_fr] = SpkRateBootstrap(sname, spk_raster, trial_length, trial_num, paras);
end % switch
...
function [win_center, fr_level] = estFRContLevel(sname, spk_raster, trl_num, trl_len, paras)
...
function sorted_ms = sortMS(usacc_props, cond, enum)

ms_cond = usacc_props(:, enum.usacc_props.condition);
sorted_ms_pos = ms_cond == cond;

sorted_ms = usacc_props(sorted_ms_pos, :);

end

function [sorted_ms, ms_num] = sortMSContLevel(sname, usacc, ~)

Trial = BCTTrial(sname);
enum = Trial.enum;
lum_level = Trial.LuminanceLevel;   % luminance levels
nlevel = length(lum_level);

sorted_ms = cell(1, nlevel);
ms_num = zeros(1, nlevel);
for k = 1:nlevel
    sorted_usacc_k = sortMS(usacc, k, enum);
