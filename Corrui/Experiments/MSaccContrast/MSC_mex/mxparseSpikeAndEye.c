...
* contrast experiment
*
* Copyright 2009 - 2011 Richard J. Cui. Created: Sun 11/20/2011 10:50:33.569 PM
* $Revision: 0.2 $  $Date: Thu 11/24/2011  1:36:04.115 PM $
*
...
typedef struct SpikeAndEye
{
	...
	unsigned short	data_info;			// information of the data
	unsigned short	max_eye_res;		// maximum resolution of eye position
	unsigned long	time[MAXLENGTH];	// time stamps/instants of data_unit
	unsigned long	spike_time[MAXLENGTH];	// time stamps of spikes
	...
	unsigned long	stim_tON1[MAXLENGTH];		//
	unsigned long	stim_tOFF1[MAXLENGTH];		//
	unsigned short	stim_x2[MAXLENGTH];			//
	unsigned short	stim_y2[MAXLENGTH];			//
	unsigned long	stim_tON2[MAXLENGTH];		//
	unsigned long	stim_tOFF2[MAXLENGTH];		//
	...
	)
{
	...
	unsigned short	c;
	short	sdata_type;
	long	fpointer, nextchunkstart,ldata_type;
