...
% MSCLED.ANALYZE_EYE_MOVEMENTS
% 
% Last modified by Richard J. Cui on Wed 10/30/2013  9:48:40.981 AM
%
...
mld_ms = MLDUsacc(this, sname);
mld_sa = MLDSacc(this, sname);
mld_bk = MLDBlink(this, sname);
mld_fx = MLDFixation(this, sname);
mld_tr = MLDTrial(this, sname);
...
if ( LEFT && RIGHT )
    ...
    lmags = mld_ms.getmagnitude( dat.left_eyedat(:,[enum.eyedat.x, enum.eyedat.y]), lsac );
    % rmags = bct_ms.getmagnitude( dat.right_eyedat(:,[1 2]), rsac );
    rmags = mld_ms.getmagnitude( dat.right_eyedat(:,[enum.eyedat.x, enum.eyedat.y]), rsac );
    if ( S.Stage_1_Options.Remove_Monoculars )
...
% update enums
% --------------
dat.enum.usacc_props    = mld_ms.getEnum();
dat.enum.saccade_props  = mld_sa.getEnum();
dat.enum.blink_props    = mld_bk.getEnum();
dat.enum.fixation_props = mld_fx.getEnum();
dat.enum.trial_props    = mld_tr.getEnum();
...
% ---------------------------------------------------------------------
if ( LEFT )
    dat.left_usacc_props = mld_ms.getprops( dat.left_eyedat, lusac , dat.samplerate, dat.isInTrialSequence, dat.isInTrialCond, left_events, dat.enum.event );
    dat.left_saccade_props = mld_sa.getprops( dat.left_eyedat, lsac , dat.samplerate, dat.isInTrialSequence, dat.isInTrialCond, left_events, dat.enum.event );
    dat.left_blink_props = mld_bk.getprops( dat.blinkYesNo, dat.samplerate, dat.isInTrialSequence, dat.isInTrialCond, left_events, dat.enum.event);
    dat.left_fixation_props =  mld_fx.getprops( dat.blinkYesNo, dat.left_eyeflags(:,dat.enum.eyeflags.saccade), dat.isInTrial, dat.isInTrialSequence, dat.isInTrialCond, dat.samplerate, dat.left_usacc_props, dat.enum.usacc_props );
end
if ( RIGHT )
    dat.right_usacc_props = mld_ms.getprops( dat.right_eyedat, rusac , dat.samplerate, dat.isInTrialSequence, dat.isInTrialCond, right_events, dat.enum.event );
    dat.right_saccade_props = mld_sa.getprops( dat.right_eyedat, rsac , dat.samplerate, dat.isInTrialSequence, dat.isInTrialCond, right_events, dat.enum.event );
    dat.right_blink_props = mld_bk.getprops( dat.blinkYesNo, dat.samplerate, dat.isInTrialSequence, dat.isInTrialCond, right_events, dat.enum.event );
    dat.right_fixation_props =  mld_fx.getprops( dat.blinkYesNo, dat.right_eyeflags(:,dat.enum.eyeflags.saccade), dat.isInTrial, dat.isInTrialSequence, dat.isInTrialCond, dat.samplerate, dat.right_usacc_props, dat.enum.usacc_props );
end
...
if LEFT || RIGHT
    dat.trial_props = mld_tr.getprops( timestamps, dat.isInTrialCond, dat.isInTrialSequence );
end
