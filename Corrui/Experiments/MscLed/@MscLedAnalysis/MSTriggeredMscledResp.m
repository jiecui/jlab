function result_dat = MSTriggeredMscledResp(current_tag, sname, S, dat)
% MSCLEDANALYSIS.MSTRIGGEREDBLANKCTRLRESP Individual cell MS-triggered response under LED condition
% 
% Description:
%   This function calculates MS-triggered response of individual cell.  The
%   results are averaged spike rate under LED condition.  The averaged
...
% 
% Syntax:
%   result_dat = MSTriggeredMscledResp(current_tag, sname, S, dat)
% 
% Input(s):
%
% Output(s):
...
% Copyright 2013 Richard J. Cui. Created: Tue 09/03/2013  3:13:46.105 PM
% $Revision: 0.1 $  $Date: Tue 09/03/2013  3:13:46.105 PM $
%
...
% -------------------------------------------------
[sorted_usacc, usacc_num] = sortMSContLevel(curr_exp, sname, usacc);      % sort usacc into different contrast level
...
result_dat.MSTriggeredMscledResp = msresp;
...
function [fr_mean, fr_sd, fr_winc, fr_level] = SpkRateSimpleAverage(curr_exp, sname, spk_raster, trial_length, trial_num, paras)
% sname     - session name

[fr_winc, fr_level] = estFRContLevel(curr_exp, sname, spk_raster, trial_num, trial_length, paras);
...
switch baselin_method
    case 'Simple average'
        [fr_base_mean, fr_base_sd, fr_base_winc, fr_base_fr] = SpkRateSimpleAverage(curr_exp, sname, spk_raster, trial_length, trial_num, paras);
    case 'Bootstrap'
...
function [sorted_ms, ms_num] = sortMSContLevel(curr_exp, sname, usacc)

Trial = MLDTrial(curr_exp, sname);
enum = Trial.enum;
