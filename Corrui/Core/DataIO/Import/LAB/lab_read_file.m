function [samples, samplerate] = lab_read_file(filename, path)
...
fseek(fid,1,'cof') ;
f5 = fread(fid,4,'char') ;
smpf = str2num(setstr(f5')) ;
clear f5
fseek(fid,1,'cof') ;
f6 = fread(fid,100,'char') ;
calib = str2num(setstr(f6')) ;
clear f6
fseek(fid,1,'cof') ;
f7 = fread(fid,9,'char') ;
indic = setstr(f7') ;
...
[dummy , n_chnls]  = size(ch_nam_temp) ;
...
if ( exist( 'eye2_h', 'var' ) )
	...
	samples(:,2) = NaN*eye1_h;
	samples(:,3) = NaN*eye1_h;
end
