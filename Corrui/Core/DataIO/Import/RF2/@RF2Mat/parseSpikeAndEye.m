function saedata = parseSpikeAndEye(~,sae)
% PARSESPIKEANDEYE extracts information from SpikeAndEye single chunk
% 
% Syntax:
%   saedata = parseSpikeAndEye(RM,sae)
% 
% Input(s):
%   RM          - RF2Mat class
%   sae         - SpikeAndEye chunk (see chunk structure in readrf2chunks.m)
%                 .position     : postion of this chunk in raw RF data
%                 .type         : SpikeAndEye
%                 .length       : length of this chunk (bytes)
%                 .data         : data in this chunk
% 
% Output(s):
%   saedata     - Parsed SpikeAndEye data structure
%                 .time         : time instants of data_unit
%                 .data_start   : start position of data_unit
%                 .data_length  : length of data_unit block
%                 .long_flag    : whether the flag length is 2 or 4
%                 .stim_type    : stimulus type (?)
%                 .data_info    : data info (?)
%                 .max_eye_res  : maxiumn eye resolution - max # units
%                                 (both horizontal and vertical)
%                 .spike_time   : time stamps of spikes
%                 .eye.x1       : 1st eye x-position
%                 .eye.y1       : 1st eye y-pos
%                 .eye.x2       : 2nd eye x-pos
%                 .eye.y2       : 2nd eye y-pos
%                 .stim.x1      : 1st stim x-pos
%                 .stim.y1      : 1st stim y-pos
%                 .stim.tON1    : 1st stim ON time
%                 .stim.tOFF1   : 1st stim OFF time
%                 .stim.x2      : 2nd stim x-pos
%                 .stim.y2      : 2nd stim y-pos
%                 .stim.tON2    : 2nd stim ON time
%                 .stim.tOFF2   : 2nd stim OFF time
% 
% Remarks:
...
% Copyright 2009 Richard J. Cui. Created: 12/09/2009  8:41:31.388 AM
% $Revision: 0.3 $  $Date: Thu 02/10/2011  1:56:36 PM $
% 
...
CNEYE 			= 2;    % 2 shorts of data (?)

CSPIKEA 		= 20;   % one spike detected from electrode A
CNSPIKEA 		= 0;    % (?) no data, just the flag
...
if ~strcmpi(chunk_type,'SpikeAndEye')
    error('Not a %s chunk!','SpikeAndEye')
end%if

% ---------------
% parse
% ---------------
data = sae.data;    % data in this chunk without the 4-byte head
...
while k < data_length+data_start-1
    ...
    switch data_type_k
        case CEYE1      % 1st eye movements
            % eye_x1_k = typecast(data(k:k+1),'uint16');
            eye_x1_k = uint16(shortfrombytes(double(data(k:k+1))));
            k = k+2;
            % eye_y1_k = typecast(data(k:k+1),'uint16');
...
saedata.eye.x1      = eye_x1;
saedata.eye.y1      = eye_y1;
saedata.eye.x2      = eye_x2;
saedata.eye.y2      = eye_y2;

saedata.stim.x1     = stim_x1;
saedata.stim.y1     = stim_y1;
saedata.stim.tON1   = stim_tON1;
saedata.stim.tOFF1   = stim_tOFF1;

saedata.stim.x2     = stim_x2;
saedata.stim.y2     = stim_y2;
saedata.stim.tON2   = stim_tON2;
saedata.stim.tOFF2   = stim_tOFF2;
