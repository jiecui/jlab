function [mn se] = aggregate_structs( sessions, filters_to_use, curr_exp, structs_LRavg, fields_for_structs_LRavg, structs_avg, fields_for_structs_avg,...
...
if ~isempty(structs_LRavg)
    all_fields = cell(length(structs_LRavg), length(filters));
    for ifilter = filters_to_use
        for istruct=1:length(structs_LRavg)
            struct_left = sessdb( 'getsessvar', sessions{1}, ['left_' structs_LRavg{istruct} '_' filters{ifilter}] );
            if isempty(struct_left)
                all_fields{istruct,ifilter} = fields_for_structs_LRavg{istruct};
            else
            ...
            for ifield=1:length(fields)
                comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}) = [];
            end
        end
    end
    ...
    for ifilter = filters_to_use
        for istruct=1:length(structs_LRavg)
            for isubj=1:length(sessions)
                struct_left = sessdb( 'getsessvar', sessions{isubj}, ['left_' structs_LRavg{istruct} '_' filters{ifilter}] );
                struct_right = sessdb( 'getsessvar', sessions{isubj}, ['right_' structs_LRavg{istruct} '_' filters{ifilter}] );
                ...
                for ifield=1:length(fields)
                    if  isfield(struct_left,fields{ifield})&& isfield(struct_right,fields{ifield}) && all(size(struct_left.([fields{ifield}])) == size(struct_right.([fields{ifield}])))
                        ...
                        current_size = size(comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}));
                        if all(current_size(1:2) == size(data)) || all(current_size == 0)
                            comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}) = cat(3,comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}),data);
                        end
                    else
    ...
    for ifilter = filters_to_use
        for istruct=1:length(structs_LRavg)
            fields = all_fields{istruct,ifilter};
            for ifield=1:length(fields)
                if size(comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}),3) > 1
                    mn.([structs_LRavg{istruct} '_' filters{ifilter}]).(fields{ifield}) = nanmean(comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}),3);
                    se.([structs_LRavg{istruct} '_' filters{ifilter}]).(fields{ifield}) = nanstd(comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}),0,3)./...
                        sqrt(sum( ~isnan(comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter})),3));
                    if ifield == length(fields)
                        disp(['Variable ' structs_LRavg{istruct} '_' filters{ifilter} ': num = ' num2str(size(comp.(structs_LRavg{istruct}).(fields{ifield}).(filters{ifilter}),3)) ])
                    end
                end
            end
        end
...
if ~isempty(structs_avg)
    % initialize
    comp=[];
    all_fields = cell(length(structs_avg),length(filters));
    for ifilter = filters_to_use
        for istruct=1:length(structs_avg)
            
            struct = sessdb( 'getsessvar', sessions{1}, [structs_avg{istruct} '_' filters{ifilter}] );
            if isempty(struct)
                all_fields{istruct,ifilter} = fields_for_structs_avg{istruct};
            else
            ...
            for ifield=1:length(fields)
                comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}) = [];
            end
        end
    end
    ...
    for ifilter = filters_to_use
        for istruct=1:length(structs_avg)
            ...
            for isubj=1:length(sessions)
                struct = sessdb( 'getsessvar', sessions{isubj}, [structs_avg{istruct} '_' filters{ifilter}] );
                ...
                for ifield=1:length(fields)
                    if isfield(struct,fields{ifield})
                        ...
                        current_size = size(comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}));
                        if all(current_size(1:2) == size(struct.(fields{ifield})))||all(current_size == 0)
                            comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}) = cat(3,comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}),struct.(fields{ifield}));
                        end
                    else
    ...
    for ifilter = filters_to_use
        for istruct=1:length(structs_avg)
            ...
            fields = all_fields{istruct,ifilter};
            for ifield=1:length(fields)
                %             [structs_avg{istruct} ' ' fields{ifield} ' ' filters{ifilter}]
                
                if size(comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}),3) > 1
                    mn.([structs_avg{istruct} '_' filters{ifilter}]).(fields{ifield}) = nanmean(comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}),3);
                    se.([structs_avg{istruct} '_' filters{ifilter}]).(fields{ifield}) = nanstd(comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}),0,3)./...
                        sqrt(sum(~isnan( comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter})),3));
                    if ifield == length(fields)
                        disp(['Variable ' structs_avg{istruct} '_' filters{ifilter} ': num = ' num2str(size(comp.(structs_avg{istruct}).(fields{ifield}).(filters{ifilter}),3))])
                    end
                else
                    continue;
                end
...
if ~isempty(structs_LRadd)
    comp=[];
    all_fields = cell(length(structs_LRadd),length(filters));
    for ifilter = filters_to_use
